...
exec
  ...
  #behave "${BEHAVE_OPTS[@]}" "features/hashcontent.feature" -n "HashContent adds hash attribute to flowfiles" &&
  #behave "${BEHAVE_OPTS[@]}" "features/azure_storage.feature" -n "A MiNiFi instance can upload data to Azure blob storage" &&
  behave "${BEHAVE_OPTS[@]}" "features/hashcontent.feature" -n "HashContent fails if the file is empty"
